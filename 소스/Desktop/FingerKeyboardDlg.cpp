...
void CFingerKeyboardDlg::OnBnClickedBtnKeyboard()
{
	...
	((CButton *)GetDlgItem(IDC_BTN_KEYBOARD))->SetWindowTextW(_T("Finger Keyboard"));
	m_ctlCob.SetCurSel(0);
	...
	*/
	...
	if(fk==NULL)
	{
	// ((CButton *)GetDlgItem(IDC_BTN_KEYBOARD))->SetWindowTextW(_T("Keyboard Exit"));

	fk = new FingerKeyboard();
	fk->programSetUp(m_ctlCob.GetCurSel());

	// 카메라 설정
	//fk->camera->setCamera(m_ctlCob.GetCurSel());

	CRect rect;
	GetWindowRect(&rect);
	HWND hWnd = ::FindWindow( NULL, _T("Program") );
	::MoveWindow(hWnd, 1000-rect.Width()-322, 200, 640, 480, TRUE);
	...
	*/
	...
	fk = new FingerKeyboard();
	if((fk->programSetUp(m_ctlCob.GetCurSel()) == -1)){
		//Message
	...
	FingerKeyboard::selectedPaperArea.x = 0;
	FingerKeyboard::selectedPaperArea.y = 0;
	FingerKeyboard::selectedPaperArea.width = 0;
	FingerKeyboard::selectedPaperArea.height = 0;
	fk->mouseListener->resetMouseDragArea();
}
...
void CFingerKeyboardDlg::OnBnClickedBtnKeyarea()
{
	...
	cvShowImage("Button Area", FingerKeyboard::buttonImage);
